...
function build_git() {
    ...
    ./scripts/build-python-deb.sh /var/www/apt.opmu.se/apt/debian master $1/setup.py $1 none none none
}
...
grep -iEv "repoze\.who|jinja2|alembic|zope\.interface|mako|^#" requirements.txt | while read -A req; do
    ./scripts/build-python-deb.sh /var/www/apt.opmu.se/apt/debian master none $req[1] $req[2] none none
done
...
./scripts/build-python-deb.sh /var/www/apt.opmu.se/apt/debian master setup.py opmuse none \
    python3.3,ffmpeg,imagemagick,unrar /etc/opmuse/opmuse.ini --no-prefix
